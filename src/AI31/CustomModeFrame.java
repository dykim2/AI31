...
/**
 * <html> There are many different things you can customize in the game, which include the following (other settings can be changed in the
 * UserInterface): <br>
 ...
 * Specifically available cards (do you want to restrict cards for the game to a specific set and give them each a preset hand for a set number of
 * rounds?) <br>
 * Players (up to 12),<br>
 * Human players (up to 12 - if equal to or larger than the number of players not including the middle hand, it will make the game mode Friendly),
 * <br>
...
public class CustomModeFrame extends JFrame implements ActionListener, AI31Constants, ItemListener, WindowListener {
	...
	/**
	 * The field describing how much aces are worth.
	 */
	private JTextField aceValue;
	/**
	 * The label asking to choose the value of aces.
	 */
	private JLabel aceLab;
	/**
	 * The field that shows the current jack value.
	 */
	private JTextField jackValue;
	/**
	 * The label asking how much you want jacks to be worth.
	 */
	...
	class CustomModeInfo {
		 ...
		 * 
		 * @param ln
		 *            - the lowest number in the deck of cards (say the deck was from 7 to A, then the lowest number is 7)
		 * @param av
		 *            - how much aces are worth
		 * @param jv
		 *            - how much jacks are worth
		 * @param hc
		 *            - how many human players there are
		 * @param pc
		 *            - how many total players there are
		 * @param lc
		 *            - how many lives per person exist
		 * @param sc
		 *            - do players start with specific cards or not?
		 * @param uta
		 *            - will the main text area be used?
		 * @param efc
		 *            - are the face card values all equal to each other?
		 * @param spl
		 *            - the location of the human player in regards to the players arraylist (competition custom only)
		 * @param mpl
		 *            - the locations of human players in regards to the players arraylist (combination custom only)
		 * @param startLoc
		 *            - the location of the starting player in the players array
		 */
		public CustomModeInfo(int ln, int av, int jv, int hc, int pc, int lc, boolean sc, boolean uta, boolean efc, int spl, ArrayList<Integer> mpl,
				int startLoc) {
			lowNum = ln;
			aceVal = av;
			jackVal = jv;
		 ...
		 * 
		 * @param in
		 *            - the CustomModeInfo object you want to copy information from
		 */
	 ...
	 * 
	 * @param b
	 *            - a boolean (that does nothing)
	 * @deprecated Do not use this constructor. Instead, use the constructor CustomModeFrame(UserInterface) as the two need to cross-reference each
	 *             other.
	 */
	 ...
	 * 
	 * @param u
	 *            - the UserInterface you want the data to be sent to
	 */
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			UserInterface.displayError("Please do not enter characters that do not belong.");
		} catch (GameException ex) {
			UserInterface.displayException(ex, 2);
			System.exit(0);
		} catch (Exception ex) {
			UserInterface.displayException(ex, 2);
		}
	...
	private void okButton() {
		if (info.multiPlayerLoc != null) {
			info.multiPlayerLoc.clear();
		} else {
			info.multiPlayerLoc = new ArrayList<Integer>();
		}
		...
		if (info.humanCt == 1) {
			...
			for (int i = 0; i < info.playerCt; i++) { if (checkBoxes[i].isSelected()) { info.multiPlayerLoc.add(new Integer(i)); } }
		}
	...
	private void confirmTop() {
		try {
			try {
				...
				UserInterface.displayException(e, 2);
			}
			...
			if (((JRadioButton) specificCardsFeatures.get(20)).isSelected()) {
				sci.specificCardsEnabled = false;
			} else {
				sci.specificCardsEnabled = true;
			}
			 ...
			 */
			for (int i = 15; i < 19; i++) {
				if (((JCheckBox) specificCardsFeatures.get(i)).isSelected()) {
					sci.availableNums.add(i - 4);
					sci.availableFaces.add(FACE_CARDS[i - 15]);
				}
			}
			/*
			...
			if (availableCards.size() < (info.playerCt * 3)) {// enough cards for a set number of players and a middle hand
				...
				UserInterface
						.displayError("Please change your card settings to allow for at least " + extraNeededCards + " extra cards to be added.");
				return;
			}
	...
	/**
	 * Confirms the middle button options.
	 */
	private void confirmMid() {
		String[] allCards = toStringArray(availableCards);
		for (String s : removedCardsToString) {
			for (int i = 0; i < allCards.length; i++) { if (s.equals(allCards[i])) { availableCards.remove(i); } }
		}
		...
		if (e) {
			...
			for (int i = 27; i < specificCardsFeatures.size(); i++) { specificCardsFeatures.get(i).setEnabled(true); }
		}
		totalCardCount();
	}
	/**
	 * Confirms the bottom button options.
	 */
	private void confirmBottom() {
		...
		if (one.isSelected()) {
			sci.specificRoundCt = 1;
		} else if (two.isSelected()) { sci.specificRoundCt = CustomModeFrame.ALL_ROUNDS; }
		for (PlayerCard pc : sci.playerCards) {
			for (Card31 c : pc.cards) { if (c != null && c.getSuit().equalsIgnoreCase("not a card!")) { c = null; } }
		}
	...
	/**
	 * Sets whether the face cards are equal or not.
	 * @param cond true if faces are equal, false if not
	 */
	private void equalFaces(boolean cond) { info.equalFaceCards = cond; }
	/**
	 * Creates the humans and tells the user what mode they have selected.
	 */
	private void humanCount() {
		...
		if (hc >= info.playerCt) {
			hc = info.playerCt - 1;
			UserInterface.displayError("Please do not enter a number larger than " + "or equal to the player count for the human count.");
			humanCount.setText("" + hc);
		}
		info.humanCt = hc;
		if (hc == 1) {
			...
			UserInterface.displayInfo("You have selected the game mode 3: Combination.", "Selected Mode");
	...
	/**
	 * Makes sure the player count works as intended.
	 */
	private void playerCount() {
		if (playerCount.getText().length() != 0) {
			...
			if (pc < 3) {
				pc = 3;
				UserInterface.displayInfo("The player count will be truncated to 3 players.");
			...
			if (info.humanCt > 1) {
				humanCount.setText("" + info.humanCt);
				humanCount();
				needToRedo = true;
			} else if (info.playerCt == info.humanCt) {
				humanCount();
			} else if (info.playerCt < info.humanCt) { humanCount.setText("" + info.playerCt); humanCount(); needToRedo = true; }
			if (needToRedo) {}
		} else {
			UserInterface.displayError("Please enter a value for the number of players. Decimals are truncated.");
		}
	}
	/**
	 * 
	...
	private void lowCards() {
		...
		int lowNum = 0;
		try {
			lowNum = (int) Double.parseDouble(lowCardNum.getText());
		} catch (NumberFormatException e1) {
			UserInterface.displayException(e1, 2);
			lowCardNum.setText("");
			return;
		}
		if (lowNum > (16 - info.playerCt)) {
			UserInterface.displayError("Please enter a valid smaller number (16-player count or smaller) for the minimum card number.");
			lowCardNum.setText("");
			return;
		} else if (lowNum < 2) {
			UserInterface.displayError("Please do not enter a value smaller than 2 for the low card number.");
			lowCardNum.setText("");
			return;
		} else if (lowNum > 9) {
			UserInterface.displayError("Please do not enter a value larger than 9 for the low card number.");
			lowCardNum.setText("");
			return;
		}
		info.lowNum = lowNum;
	...
	private void aceValue() {
		...
		if (val < 9) {
			UserInterface.displayError("Please do not enter a value smaller than 9 for the value of each ace.");
			aceValue.setText("");
			return;
		}
		if (val > 15) {
			UserInterface.displayError("Please do not enter a value larger than 15 for the value of each ace.");
			aceValue.setText("");
			return;
		}
	...
	private void jackValue() {
		...
		if (val < 9) {
			UserInterface.displayError("Please do not enter a value smaller than 9 for the value of each jack.");
			jackValue.setText("");
			return;
		}
		if (val > 14) {
			UserInterface.displayError("Please do not enter a value larger than 14 for the value of each jack.");
			jackValue.setText("");
			return;
		}
	...
	private void lifeCount() {
		...
		if (life < 1) {
			UserInterface.displayError("Please do not enter a value smaller than 1 for the number of lives each player has.");
			lifeCount.setText("");
			return;
		}
	...
	private void startingPosition() throws GameException {
		...
		if (pos < 0) {
			UserInterface.displayError("Please do not enter an index smaller than 0 for your starting position.");
			startingPosition.setText("");
			return;
		}
		if (pos >= info.playerCt - 1) {
			UserInterface.displayError("Please do not enter an index larger than or equal to the number "
					+ "of players -1 (mid is a player) for your starting position.");
			startingPosition.setText("");
			return;
		}
	...
	private void startingIndex() {
		...
		if (val < 0) {
			UserInterface.displayError("Please do not enter a value smaller than 0 for the first starting player's index.");
			startingIndex.setText("");
			return;
		}
		if (val >= info.playerCt - 1) {
			UserInterface.displayError("Please do not enter a value larger than or equal to the number of players "
					+ "(excluding the middle hand) for the first starting player's index.");
			startingIndex.setText("");
			return;
		}
	 ...
	 * 
	 * @param index
	 *            - which player is getting the cards?
	 */
	private void addCardButton(int index) {
		...
		for (int i = 0; i < availableCards.size(); i++) {
			if (availableCards.get(i).toString().equals(select)) {
				if (sci.playerCards.get(index).cards[0] == null || sci.playerCards.get(index).cards[0].getSuit().equalsIgnoreCase("not a card!")) {
					sci.playerCards.get(index).cards[0] = availableCards.get(i);
				} else if (sci.playerCards.get(index).cards[1] == null
						|| sci.playerCards.get(index).cards[1].getSuit().equalsIgnoreCase("not a card!")) {
					sci.playerCards.get(index).cards[1] = availableCards.get(i);
				} else if (sci.playerCards.get(index).cards[2] == null
						|| sci.playerCards.get(index).cards[2].getSuit().equalsIgnoreCase("not a card!")) {
					sci.playerCards.get(index).cards[2] = availableCards.get(i);
				} else {
					UserInterface.displayError("Please do not attempt to add more than 3 cards for a player.");
					return;
				}
		...
		for (int i = 0; i < Card31.RANDOM_SUIT_STRINGS.length; i++) {
			if (Card31.RANDOM_SUIT_STRINGS[i].equals(select)) {
				if (sci.playerCards.get(index).cards[0] == null || sci.playerCards.get(index).cards[0].getSuit().equalsIgnoreCase("not a card!")) {
					sci.playerCards.get(index).cards[0] = new Card31(randomCount, Card31.RANDOM_SUIT_STRINGS[i]);
					randomCount--;
				} else if (sci.playerCards.get(index).cards[1] == null
						|| sci.playerCards.get(index).cards[1].getSuit().equalsIgnoreCase("not a card!")) {
					sci.playerCards.get(index).cards[1] = new Card31(randomCount, Card31.RANDOM_SUIT_STRINGS[i]);
					randomCount--;
				} else if (sci.playerCards.get(index).cards[2] == null
						|| sci.playerCards.get(index).cards[2].getSuit().equalsIgnoreCase("not a card!")) {
					sci.playerCards.get(index).cards[2] = new Card31(randomCount, Card31.RANDOM_SUIT_STRINGS[i]);
					randomCount--;
				} else {
					UserInterface.displayError("Please do not attempt to add more than 3 cards for a player.");
					return;
	...
	private void removeCardButton(int index) {
		...
		/*
		 * Temporarily commenting it out to find a more efficient solution if(!sci.playerCards.get(index).cards[ind].isRandomCard()) {//if it is not a
		 * random card
		 * 
		 * int[] realNums = AI31Functions.convertInts(sci.availableNums); String[] realSuits = sci.availableSuits.toArray(new
		 * String[sci.availableSuits.size()]); ArrayList<Card31> removedCards = new ArrayList<Card31>(); for(PlayerCard c : sci.playerCards)
		 * {if(c!=null) {for(Card31 card : c.cards) { if(card!=null && !card.getSuit().equalsIgnoreCase("not a card")) {removedCards.add(card);}}}}
		 * removedCards.remove(sci.playerCards.get(index).cards[ind]); availableCards.clear(); char[] possibleFaces = new
		 * char[sci.availableFaces.size()]; for(int i=0; i<sci.availableFaces.size(); i++) {possibleFaces[i] = sci.availableFaces.get(i);}
		 * availableCards.addAll(Card31.generateCards(realSuits, realNums, possibleFaces, removedCards.toArray(new Card31[4]))); totalCardCount(); }
		 */
		...
		if (sci.playerCards.get(index).cards[ind].isRandomCard()) {
	...
	private void pIndexBox() {
		((JButton) specificCardsFeatures.get(29))
				.setText("Add a Card (Player #" + ((JComboBox) specificCardsFeatures.get(28)).getSelectedIndex() + ")");
		((JButton) specificCardsFeatures.get(30))
				.setText("Remove a Card (Player #" + ((JComboBox) specificCardsFeatures.get(28)).getSelectedIndex() + ")");
	...
	private void removeACard() {
		...
		try {
			if (counter == sci.availableNums.size() - 1) {
				throw new GameException("Plesae do not attempt to remove the last card of a suit.", "F0319");
			}
		} catch (GameException e) {
			UserInterface.displayException(e, 2);
		}
		if (!find(removedCardsToString, (String) (box.getSelectedItem()))) {
			removedCardsToString.add((String) (box.getSelectedItem()));
			UserInterface.displayInfo("You have removed the card " + ((String) (box.getSelectedItem())) + " from the list of available cards.",
					"Card removal successful.");
		} else {
			removedCardsToString.remove((String) (box.getSelectedItem()));
			UserInterface.displayInfo("You have added back the card " + ((String) (box.getSelectedItem())) + " to the list of available cards.",
					"Card re-addition successful.");
		}
	...
	private void howManyCards() {
		...
		while (!conditions) {
			...
			if (status == JOptionPane.NO_OPTION) {
				for (int i = 0; i < 4; i++) {
					if (AI31Constants.ORDERED_SUITS[i].equalsIgnoreCase((String) (suitBox.getSelectedItem()))) {
						sci.suitQuantities[i] = antiQuantity;
					}
				}
				// reset the quantity
				return;
			...
			if ((Integer) (numBoxTwo.getSelectedItem()) >= (Integer) (numBoxOne.getSelectedItem())) {
				conditions = true;
			} else {
				UserInterface.displayError("Please enter a larger max (second term) than min (first term) for the selected suit.");
			}
		}
		String selectSuit = ((String) suitBox.getSelectedItem()).toLowerCase();
		if (selectSuit.contains("club")) {
			sci.suitQuantities[0] = new SuitRandom((Integer) numBoxOne.getSelectedItem(), (Integer) numBoxTwo.getSelectedItem());
		} else if (selectSuit.contains("diamond")) {
			sci.suitQuantities[1] = new SuitRandom((Integer) numBoxOne.getSelectedItem(), (Integer) numBoxTwo.getSelectedItem());
		} else if (selectSuit.contains("heart")) {
			sci.suitQuantities[2] = new SuitRandom((Integer) numBoxOne.getSelectedItem(), (Integer) numBoxTwo.getSelectedItem());
		} else if (selectSuit.contains("spade")) {
			sci.suitQuantities[3] = new SuitRandom((Integer) numBoxOne.getSelectedItem(), (Integer) numBoxTwo.getSelectedItem());
		}
	...
	private void numRounds() {
		sci.specificRoundCt = Integer.parseInt(UserInterface
				.displayQuestionIgnoreNull("Entering a negative number will result in an infinite number of rounds.", "How many rounds?"));
		if (sci.specificRoundCt < 0) {
			((JLabel) specificCardsFeatures.get(37)).setText("Number of selected rounds: All");
		} else {
			((JLabel) specificCardsFeatures.get(37)).setText("Number of selected rounds: " + sci.specificRoundCt);
		}
	}
	...
	private void setUpPrimaryFrame() {
		...
		pInfo.setToolTipText(
				"The player count includes the middle hand. Note that changing the player count will deselect all human locations for multiplayer.");
		...
		startPos.setToolTipText(
				"Would you like to start in front or behind a specific player? If so, what number index do you want to be placed in?");
		startPos.setEnabled(false);
		...
		humanLocsInfo
				.setToolTipText("Where (in terms of indexes) do you want the humans to be located (in front of or behind a specific player index)?");
		humanLocsInfo.setEnabled(false);
		...
		eqFace.setToolTipText(
				"<html>Equal faces mean that jack-queen-king values are equal.<br>Not equal faces means that jack-queen-king values increment.<html>");
		...
		for (Component comp : primaryPanel.getComponents()) {
			if (comp instanceof JPanel) {
				for (Component com : ((JPanel) comp).getComponents()) { generalComponents.add((JComponent) com); }
			} else {
				generalComponents.add((JComponent) comp);
			}
		}
	...
	private void setUpSecondaryFrame(GridBagConstraints c) {
		...
		cardBox.setToolTipText(
				"<html>Choose cards you don't want to play with. Each time you remove a card (by clicking on it), it will be saved<br>and visible "
		...
		cardQuantities.setToolTipText("<html>How many cards do you want per suit? Do you want an even distribution or not?<br>"
				...
				+ "If you add too few cards, the rest are random; If you add too many cards...<br>" + "then the first requests will take priority.");
		...
		for (int i = 0; i < specificCardsFeatures.size(); i++) {
			...
			if (specificCardsFeatures.get(i) instanceof AbstractButton) {
				System.out.println(((AbstractButton) specificCardsFeatures.get(i)).getText());
			}
		}
		pack();
	...
	private void disableCheckBoxes() {
		for (int i = 0; i < info.playerCt - 1; i++) { if (!checkBoxes[i].isSelected()) { checkBoxes[i].setEnabled(false); } }
	}
	...
	/**
	 * Converts the elements in the given array list into an array containing the object's string representation.
	 * 
	 * @param al
	 *            the array list containing the objects
	 * @return a string array containing the string representation of all of the
	 */
	...
	/**
	 * Converts the elements in the given array into an array containing each object's string representation.
	 * 
	 * @param ar
	 *            the array
	 * @return
