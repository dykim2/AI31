...
public class UserInterface extends JFrame implements AI31Constants {
	...
	public static String displayQuestion(Object body, String title) {
		...
		try {
			if (result == null) { throw new NullPointerException("You must enter a value for the result. Error #N8437"); }
	...
	public static String displayQuestionIgnoreNull(Object body, String title) {
		return JOptionPane.showInputDialog(null, body, title, JOptionPane.QUESTION_MESSAGE);
	}
	/**
	...
	protected void startGame() {
		...
		try {
			...
			if (errorFound) { displayError("What is this error?"); }
		}
		...
		try {
			...
			if (names.size() == 0) {
				...
				if (decision == 0) {
					while (names.size() < playerCount - 1) {
						...
						do {
							...
							if (name == null || name.equals("") || name.equals("-1")) {
								...
								displayError("Please do not enter a duplicate AI name.");
							}
						} while (!checkAI(name));
						if (name == null || name.equals("") || name.equals("-1")) { break; }
						if (names.get(names.size() - 1).equals("-1")) {
					...
					for (int i = 0; i < playerCount - 1; i++) { players.add(new Machine(i, this, generateName())); }
				}
			} else {
			...
			if (d == JOptionPane.NO_OPTION) {
				players.add(new Human(humanName, this));
			} else {
				players.add(new Machine(players.size(), this, humanName));
			}
		...
		try {
			...
			if (gameMode.equalsIgnoreCase("friendly")) {
					...
					do {
						...
						if (name != null && !name.equals("") && !name.equals("-1")) {
							if (checkName(name)) {
								...
								displayError("Please do not enter a duplicate human name.");
							}
						} else {
				...
				if (decision == 0) {
					...
					for (int i = 0; i < playerCount - humanCount; i++) { players.add(new Machine(i, this, generateName())); }
					for (int i = 0; i < humanCount; i++) { players.add(new Human(humanPlayerNames.get(i), this)); }
				}
				displayInfo("31 is a game of swapping cards to gain the highest value in cards." + "\nYou will be facing "
						+ (playerCount - humanCount) + " highly skilled AI(s) and " + (humanCount - 1) + " human player(s)!"
	...
	protected void setUpCustom() {
		...
		try {
			...
			if (playerCount - 1 == humanCount) {
				gameMode = "friendly";
				multiPlayer = true;
			} else if (humanCount == 1) {
				gameMode = "competition";
				humanPlayer = customInfo.singlePlayerLoc;
				multiPlayer = false;
			} else {
		...
		if (multiPlayer) {
			if (gameMode.equalsIgnoreCase("Friendly")) {
				for (int i = 0; i < playerCount - 1; i++) {
					String name;
					do {
						name = displayQuestionIgnoreNull("Enter the name of a human player or -1 to randomize the name.",
								"Custom human name choice!");
						if (name != null && !name.equals("") && !name.equals("-1")) {
							if (checkName(name)) {
								humanPlayerNames.add(name);
								break;
			...
			for (int i = 0; i < playerCount - 1; i++) {
				if (i == customInfo.singlePlayerLoc) {
					...
					if (decision1 == JOptionPane.YES_OPTION) {
						...
						if (name.equals("") || name.equals("-1")) {
							name = generateName();
							panel.addText("Welcome to 31! Your name is: " + name + ".\n");
						}
					} else {
						name = generateName();
						panel.addText("Welcome to 31! Your name is: " + name + ".\n");
	...
	protected void addAINames() {// gives the AIs generic names (or some to my friends),
		...
		playerNames.add("Brian");
		playerNames.add("Adam");
		playerNames.add("John");
		playerNames.add("Daniel");
		playerNames.add("Jessica");
		playerNames.add("Katie");
		playerNames.add("Mark");
		playerNames.add("Ryan");
		playerNames.add("Colin");
		playerNames.add("Kelly");
		playerNames.add("Christopher");
		playerNames.add("Ashley");
		playerNames.add("Rachel");
		playerNames.add("Elizabeth");
		playerNames.add("Emily");
		playerNames.add("Abigail");
		playerNames.add("Sara");
		playerNames.add("David");
		playerNames.add("Joshua");
		playerNames.add("Samantha");
		playerNames.add("Hannah");
		playerNames.add("Lauren");
		playerNames.add("James");
		playerNames.add("Taylor");
		playerNames.add("Benjamin");
		playerNames.add("Jackson");
		playerNames.add("Michael");
		playerNames.add("Stephanie");
		playerNames.add("Emma");
		playerNames.add("Gavin");
		playerNames.add("Nathan");
		playerNames.add("Madison");
		playerNames.add("Liam");
		playerNames.add("Robert");
		playerNames.add("Sophia");
		playerNames.add("Olivia");
		playerNames.add("Isabella");
		playerNames.add("Isabel");
		playerNames.add("Evan");
		playerNames.add("Alex");
		playerNames.add("Kayla");
		playerNames.add("Audrey");
		playerNames.add("Frank");
		// below are my friends names, why not?
		playerNames.add("Hayk");
		playerNames.add("Elise");
		playerNames.add("Conall");
		playerNames.add("Logan");
		playerNames.add("Marina");
		playerNames.add("Fortune David");
		playerNames.add("Grayson");
		playerNames.add("Dong-Yon"); // playerNames.add("Kaiya"); playerNames.add("Moiz");
		playerNames.add("Mika");
		playerNames.add("Clay");
	 ...
	 */
	protected void deal() throws GameException {
		...
		try {
			...
			if (players.get(playerTurn).getClass().getName().equals("AI31.Human")) {
				...
				if (((Machine) players.get(playerTurn)).doDealersTurn() == 1) {
					...
					panel.addText(players.get(playerTurn).getName() + ": I want to keep the cards I was dealt.\n");
	...
	protected void gameplay() throws GameException {// each run of gameplay is a turn of a player but with caught exceptions
		...
		try {
			if (players.get(playerTurn) != null) { processTurns(); }
		} catch (PlayingCardException e) {
			displayException(e, 0);
			try {
				throw new GameException("Something went wrong swapping cards.", "P8732");
	...
	protected void processTurns() throws GameException, PlayingCardException {
		...
		if (decision > 0) {
			...
			if (decision == -2) {
				...
				throw new GameException("A decision must be made. Please try again.", "O5328");
			} else {/* This should not run, but if it does, I will throw an exception */
				throw new GameException("An error occured trying to pass.", "O7742");
			}
			moveTurnCounter();
		}
		panel.addText(players.get(players.size() - 1) + "\n");
		...
		try {
			...
			for (int i = 0; i < players.size() - 1; i++) {
				if (players.get(i) != null) {
					handSums[i] = players.get(i).sumOfHand();
				} else {
					handSums[i] = 100;
				}
			}
			...
			for (int i = 0; i < livesLostPlayers.size(); i++) {
				...
				if (livesLostPlayers.get(i).getLives() == 0) {
					...
					alivePlayerCount--;
					if (panel.getEliminationStatus() == UserPanel.LIVES_AND_ELIMINATIONS
							|| panel.getEliminationStatus() == UserPanel.NO_LIVES_AND_ELIMINATIONS) {
				...
				if (multiPlayer) {
					for (int k = 0; k < players.size() - 1; k++) {
						if (players.get(k) != null) {
							if (players.get(k).getClass().getName().equals("AI31.Human") && players.get(k).getLives() != 0) {
								humansAlive = true;
								break;
							}
						}
	...
	/**
	 * <html> Determines whether the game is over.
	 * 
	 * @return true if the game is over (all human players or your AI are eliminated), <br>
	 *         false if the game is stll running </html>
	 */
	...
	protected void swapWithMiddle() {
		...
		try {
			...
			if (players.get(playerTurn).getClass().getName().equals("AI31.Human") && !multiPlayer) {
				panel.addText(players.get(playerTurn).getName() + ": " + players.get(playerTurn));
				panel.updatePlayerHand();
			} else {
				panel.updateSpecificPlayerHand();
			}
	...
	protected void dealCards() {
		...
		if (specificInfo != null && specificInfo.specificCardsEnabled && specificInfo.specificRoundCt != 0) {
			...
			if (suitQuantitiesInfo) {
				...
				for (int i = 0; i < 4; i++) {
					if (specificInfo.suitQuantities[i] != customFrame.antiQuantity) {
						specificInfo.suitQuantities[i].randomize();
						System.out.println(specificInfo.suitQuantities[i] + "\n");
					}
				}
				for (int i = 0; i < players.size(); i++) {
					...
					if (addCards.size() < totalCards) {// more cards can be added
						if (specificInfo.suitQuantities[i] != customFrame.antiQuantity) {// not an antiquantity
							for (int k = 0; k < specificInfo.suitQuantities[i].randomVal; k++) {// each extra card to add
								try {
									addCards.add(generateSuitCard(AI31Constants.ORDERED_SUITS[i]));
								} // generate a suit card
								catch (GameException e) {
									displayException(e, 0);
								}
							}
	...
	/**
	 * <html> Generates a random card based on a suit or set of suits, specified by the condition variable. <br>
	 * 0: Spades <br>
	 * 1: Clubs <br>
	 * 2: Hearts <br>
	 * 3: Diamonds <br>
	...
	protected Card31 generateSuitCard(int condition) throws GameException {
		...
		while (true) {
			...
			for (String s : chosenSuits) {
				if (s.equalsIgnoreCase(cards.get(rand).getSuit())) { return cards.remove(rand); }
				Collections.shuffle(chosenSuits);
			}
	...
	protected int findPlayer(Player p) {// where will I use this?
		for (int i = 0; i < players.size() - 1; i++) {
			if (players.get(i) != null && players.get(i) == p) {
	 ...
	 *            "combination" means some AIs, some humans </html>
	 */
	protected void setMode(String mode) {
		...
		if (mode.equals("") || mode == null || !okMode) {
			try {
				throw new GameException("Please do not attempt to use a nonexistent game mode.", "I4204");
			} catch (GameException e) {
				displayException(e, 0);
				displayError("Please choose a different mode next time.");
				System.exit(0);
			} ;
		} else {
			gameMode = mode;
	...
	protected void setHumanCount(int ct) {
		if (ct < 0 || ct > playerCount) {
			displayError("Please do not attempt to add an invalid number of players.");
		} else {
			humanCount = ct;
		}
	}
