 ...
 * @since 1.0
 */
public class PlayingCard {// this assumes the running of one game at a time, not two simultaneously (one after the other is
							// fine)
	/**
	...
	/**
	 * A specific number used to describe face cards (J, Q, K, or A) for image and other purposes. Aces are 14, kings 13,
	 * queens 12, and jacks 11. If the card is not a face card the pip value is the special number.
	 */
	...
	/**
	 * A default image, used as a basis for the card's width and height
	 */
	private static BufferedImage defaultImage = null;
	/**
	 * The width of the card image.
	 */
	public static int cardImageWidth;
	/**
	 * The height of the card image.
	 */
	public static int cardImageHeight;
	...
	public PlayingCard(char f, String s) {
		try {
			...
			if (f == 'A' || f == 'a') {
				...
				throw new PlayingCardException(f + " is not a valid face.", "C4854");
			}
			suit = new String(s);
			setImage();
	 ...
	 * 
	 * @param choice - 1 for determining whether aces are high (true) or not (false), 0 for determing whether face cards
	 *            increment (true) or not (false)
	...
	public static boolean getValueConditions(int choice) throws PlayingCardException {
		if (choice == 1) {
			...
			return faceCardsIncrement;
		} else {
			throw new PlayingCardException(choice + " is not a valid choice.", "V4009");
		}
	}
	/**
	 ...
	 * @param condition - true for aces: if aces are high or not (true means aces high), true for faces: whether the face
	 *            cards increment (11-12-13) or stay the same (10-10-10) (true means faces increment)
	 * 
	...
	public static void setValueConditions(int choice, boolean condition, int val) throws PlayingCardException {
		if (choice == 1) {
			aceHigh = condition;
			previousAceValue = aceValue;
			aceValue = val;
		} else if (choice == 0) {
			faceCardsIncrement = condition;
			previousJackValue = jackValue;
			jackValue = val;
		} else {
			throw new PlayingCardException(choice + "is not a valid choice.", "B1424");
		}
	}
	/**
	...
	public void updateNumber() throws PlayingCardException {
		if (number == previousAceValue) {
			number = aceValue;
		} else if (number == previousJackValue + 2) {
			number = jackValue;
		} else if (number == previousJackValue + 1) {
			number = jackValue;
			...
			if (face == 'K' || face == 'k') {
				...
				throw new PlayingCardException(face + " is not a valid face.", "1531");
			}
		}
	...
	/**
	 * Sets the image of the card to the given image.
	 * 
	...
	public void setImage() {
		...
		try {
			BufferedImage image = ImageIO.read(getClass().getResource("/Pictures/" + add + ".png"));// reads the image
	...
	/**
	 * Sets the default image, used to define the width and height of the image. This will be used in an attempt to drag the
	 * cards around.
	 */
	private void setDefaultImage() {
		try {
			defaultImage = ImageIO.read(getClass().getResource("/Pictures/3H.png"));
			cardImageWidth = defaultImage.getWidth();
			cardImageHeight = defaultImage.getHeight();
		} catch (IOException e) {
			displayException(e);
		}
	}
	...
	public Image recieveImage() {
		...
		try {
			return ImageIO.read(getClass().getResource("/Pictures/" + add + ".png")).getScaledInstance(-2, 100,
					Image.SCALE_SMOOTH);
		} catch (IOException e) {
			displayException(e);
			return null;
	...
	/**
	 * <html> Compares two cards and returns an int that represents the card that has the larger value.
	 * 
	 ...
	 *         This only compares on the basis of number. </html>
	 */
	public int compareByNumber(PlayingCard c) {
		...
		if (this == c || this.number == c.number) {
			return 0;
	...
	/**
	 * <html> Compares two cards and returns a boolean checking if the card's faces are the same.s
	 * 
	 * @param c - the second PlayingCard you want to compare this PlayingCard against
	 * @return true when the two cards have the same face and <br>
	 *         false when the two cards have different face. <br>
	 *         This only checks if the two cards have the same face, nothing else. </html>
	 */
	...
	/**
	 * <html> Compares two cards and returns a boolean checking if the card's suits are the same.
	 * 
	 * @param c - the second PlayingCard you want to compare this PlayingCard against
	 * @return true when the two cards have the same suit and <br>
	 *         false when the two cards have different suit. <br>
	 *         This only checks if the two cards have the same suit, nothing else. </html>
	 */
	public boolean compareBySuit(PlayingCard c) {
		...
		if (this.suit.equals(c.suit) || this == c) {
			return true;
		} else {
			return false;
		}
	...
	public boolean equals(Object o) {
		if (o.getClass().getName().equals("baselineCustomClasses.PlayingCard")) {
			...
			if ((c.getNumber() == this.getNumber() && c.getSuit().equals(this.getSuit())) && c.getFace() == this.getFace()) {
				return true;
			} else {
				return false;
			}
		} else {
			throw new ClassCastException(o.getClass().getName() + " cannot be cast to a PlayingCard object.");
		}
	}
	/**
	...
	public static char largestFace() {
		if (!faceCardsIncrement) {
			if (aceValue > jackValue) {
				return 'A';
			} else if (aceValue < jackValue) {
				return 'K';
			} else {
				return 'A';
			}
		} else {
			if (aceValue > jackValue + 3) {
				return 'A';
			} else if (aceValue < jackValue + 3) {
				return 'K';
			} else {
				return 'K';
			}
	 ...
	 * @return the result of the question
	 */
	protected static String displayQuestionIgnoreNull(Object body, String title) {
	 ...
	 */
	protected static void displayError(Object body) {
	 ...
	 * @param body the information to display
	 * @param title the title of the display pop up
	 * @return an integer replacing the choice made by the user
	...
	//checking if the mouse is over this playing card
	protected boolean mouseOverThisCard() {
		//get the mouse location
		//then get the location of the card coordinates wise from the user interface
		return false;
	}
}
